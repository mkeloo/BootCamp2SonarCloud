...
try {
  ...
  fs.readFile('listings.json', 'utf8', function (err, data) {
    ...
    for (const listing of listingData) {
      // check if cordinates exist and extract them for each listing
      if (listing.coordinates) {
        listing.latitude = listing.coordinates.latitude;
        listing.longitude = listing.coordinates.longitude;
      }

      // save the listing to the db using create func
