...
async function removeCable() {
  ...
  try {
    ...
    console.log('Removed CABL listing.');
  } catch (error) {
    console.error('Error removing CABL listing:', error);
  }
}
...
async function addDSIT() {
  ...
  try {
    ...
    await Listing.create(dsit);
    console.log('Added DSIT listing.');
  } catch (error) {
    console.error('Error adding DSIT listing:', error);
  }
}
    ...
    await Listing.update(
      {
        ...
        latitude: 29.6446016,
        longitude: -82.3487929,
      },
      { where: { name: 'Phelps Laboratory' } }
    );
    console.log('Updated Phelps Lab address.');
  } catch (error) {
    console.error('Error updating Phelps Lab address:', error);
  }
}
